...
def filter_gulf_aircraft(states, selected_country=None):
       filtered = []
       for state in states:
           ...
           if origin_country in GULF_COUNTRIES:
               if selected_country is None or origin_country == selected_country:
                   app.logger.debug(f"Matched aircraft: {callsign}, Country: {origin_country}")
                   aircraft_data = {field: state[idx] for idx, field in enumerate(FIELDS)}
                   filtered.append(aircraft_data)
       app.logger.debug(f"Filtered {len(filtered)} aircraft for country: {selected_country or 'All'}")
       return filtered
...
def get_aircraft():
       ...
       filtered_data = filter_gulf_aircraft(states, country)
       if not filtered_data:
           message = f"No aircraft found for {country or 'Gulf countries'}"
